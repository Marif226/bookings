...
func TestForm_Valid(t *testing.T) {
	r := httptest.NewRequest("POST", "/nevermind", nil)
	form := New(r.PostForm)

	isValid := form.Valid()
	if !isValid {
		t.Error("get invalid where should be valid")
	}
}
...
func TestForm_MinLength(t *testing.T) {
	...
	if form.Valid() {
		t.Error("shows min length of 100 net when data is shorter")
	}
